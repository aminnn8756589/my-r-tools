# ====================== INSPECTION PROFESSIONNELLE - VERSION PASSEPARTOUT ======================
# Fonction : inspection()
# Robuste, adaptée à R Markdown et aux grands jeux de données
...
inspection <- function(df, title = "Rapport d'Inspection des Données") {
  ...
  cat(strrep("=", 95), "\n")
  cat("               ", title, "\n")
  cat("               ", nrow(df), "observations ×", ncol(df), "variables\n")
  cat(strrep("=", 95), "\n\n")
  ...
  cat("Aperçu des données et types :\n")
  glimpse(df)
  cat("\n")
  
  # 2. Doublons (corrigé avec everything())
  cat("**2. DOUBLONS**\n\n")
  dupes <- janitor::get_dupes(df, everything())
  ...
  if (nrow(dupes) == 0) {
    ...
    cat("Exemple des premières lignes dupliquées :\n")
    print(head(dupes, 6))
    cat("\n")
  }
  
  # 3. Tableau récapitulatif variable par variable (corrigé et très lisible)
  cat("**3. RÉCAPITULATIF VARIABLE PAR VARIABLE**\n")
  cat("(Trié par % de valeurs manquantes décroissant)\n\n")
  ...
  if (length(num_vars) > 0) {
    cat("Variables numériques :", paste(num_vars, collapse = ", "), "\n")
  }
  cat("→ Analyse détaillée des outliers disponible dans une fonction de nettoyage séparée.\n\n")
  
  cat(strrep("=", 95), "\n")
  cat("FIN DU RAPPORT D'INSPECTION\n")
  cat("Aucune modification n'a été apportée à votre dataframe.\n")
  cat(strrep("=", 95), "\n\n")
  
  # Retour invisible (utile dans R Markdown)
  invisible(list(
    dimensions = dim(df),
    recap_table = recap,
